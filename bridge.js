...
async function _doInit() {
  ...
  try {
    ...
    if (bundleJson) {
      ...
      // Load sim config separately (not in bundle)
      window.IVP_SIM_CONFIG = await getSimConfig();
      _buildGoogleScriptRunShim();
      return; // Data loaded from session — done
    }
  } catch (e) {
  ...
  // This happens when a module HTML is opened directly (not via taskpane dialog)
  try {
    window.IVP_CONFIGS = await getConfigsAll();
    window.IVP_CFG     = await getActiveConfig();

    if (window.IVP_CFG && window.IVP_CFG.config) {
      window.IVP_BUNDLE = await buildBundle(window.IVP_CFG.config);
    } else {
    ...
    console.error('[bridge] _doInit fallback failed:', e);
    window.IVP_BUNDLE     = null;
    window.IVP_SIM_CONFIG = { apiKey: '', apiModel: 'claude-sonnet-4-20250514', maxTokens: 1000 };
  }
...
function _buildGoogleScriptRunShim() {
  ...
  function makeRunner(asyncFn) {
    return function runner(...args) {
      asyncFn(...args)
        .then(result  => { if (runner._success) runner._success(result);  })
        .catch(err    => { if (runner._failure) runner._failure(err); else console.error('[bridge] Unhandled:', err); });
    };
  }
  ...
  window.google.script = {
    run: new Proxy(shimRoot, handler),
    host: {
      close: () => {
